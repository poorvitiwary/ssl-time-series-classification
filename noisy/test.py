...
def test_model(num_runs=5):
    """Test models matching 'noisy_model_{size}.pth' pattern.
    ...
    model_name = f"noisy_model_{size}.pth"
    model_path = os.path.join(MODEL_SAVE_PATH, model_name)
    
    if not os.path.exists(model_path):
        print(f"Error: Model not found: {model_name}")
        print("Available noisy models:")
        for f in os.listdir(MODEL_SAVE_PATH):
            if f.startswith("noisy_model_") and f.endswith(".pth"):
                print(f"- {f}")
        return
