...
def test_model(num_runs=5):
    """Test models matching 'supervised_model_{size}.pth' pattern.
    ...
    model_name = f"supervised_model_{size}.pth"
    model_path = os.path.join(MODEL_SAVE_PATH, model_name)
    
    if not os.path.exists(model_path):
        print(f"Error: Model not found: {model_name}")
        print("Available supervised models:")
        for f in os.listdir(MODEL_SAVE_PATH):
            if f.startswith("supervised_model_") and f.endswith(".pth"):
                print(f"- {f}")
        return
