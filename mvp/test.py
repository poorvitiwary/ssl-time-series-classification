...
def test_model(num_runs=5):
    """Test models matching 'mvp_model_{size}.pth' pattern.
    ...
    model_name = f"mvp_model_{size}.pth"
    model_path = os.path.join(MODEL_SAVE_PATH, model_name)
    
    if not os.path.exists(model_path):
        print(f"Error: Model not found: {model_name}")
        print("Available mvp models:")
        for f in os.listdir(MODEL_SAVE_PATH):
            if f.startswith("mvp_model_") and f.endswith(".pth"):
                print(f"- {f}")
        return
    ...
    for i in range(num_runs):
        preds, targets = learn.get_preds(dl=test_dl.train)
